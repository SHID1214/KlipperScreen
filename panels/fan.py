...
class Panel(ScreenPanel):
    ...
    def add_fan(self, fan):
        ...
        # --- 팬 이름 번역 로직 시작 ---
        if "Exhaust_fan" in fan:
            fan_name = _("Exhaust Fan")
        elif "Parts_fan" in fan:
            fan_name = _("Parts Fan")
        elif "controller_fan" in fan:
            fan_name = _("Controller Fan")
        elif "hotend_fan" in fan:
            fan_name = _("Hotend Fan")
        elif "Auxiliary_fan" in fan:
            fan_name = _("Auxiliary Fan")
        else:
            # 매칭되는 이름이 없으면 원래 이름 사용
            fan_name = fan.split()[1] if len(fan.split()) > 1 else fan
        # --- 팬 이름 번역 로직 끝 ---
