...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        # 아이콘 크기 설정 (화면 너비의 20%)
        icon_size = self._screen.width / 300

        # 1. 버튼 생성 (아이콘 크기 인자 추가: icon_size)
        estop = self._gtk.Button("emergency", _("Emergency Stop"), "color2", icon_size)
        estop.connect("clicked", self.emergency_stop)

        poweroff = self._gtk.Button("shutdown", _("Shutdown"), "color1", icon_size)
        poweroff.connect("clicked", self.reboot_poweroff, "shutdown")

        restart = self._gtk.Button("refresh", _("Restart"), "color3", icon_size)
        restart.connect("clicked", self.reboot_poweroff, "reboot")

        # (Restart KlipperScreen과 Lock 버튼은 생성하지 않음)

        # 2. 그리드 레이아웃 설정
        self.main = Gtk.Grid(row_homogeneous=True, column_homogeneous=True)
        
        # Emergency Stop: 프린터 상태가 정상일 때만 0열에 배치 (너비 1, 높이 2)
        if self._printer and self._printer.state not in {'disconnected', 'startup', 'shutdown', 'error'}:
            self.main.attach(estop, 0, 0, 1, 2)
        
        # Shutdown: 1열에 배치 (너비 1, 높이 2)
        self.main.attach(poweroff, 1, 0, 1, 2)
        
        # Restart: 2열에 배치 (너비 1, 높이 2)
        self.main.attach(restart, 2, 0, 1, 2)
